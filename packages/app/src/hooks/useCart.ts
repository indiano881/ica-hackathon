...
/**
 * Hook for managing the active cart in Couchbase Lite.
 *
 * Pass userId and storeId to create a new cart if none exists.
 * Pass no args to load the current cart from state.
 */
export function useCart(userId?: string, storeId?: string) {
  const [cart, setCart] = useState<Cart | null>(null);

  useEffect(() => {
    if (userId && storeId) {
      loadOrCreateCart(userId, storeId);
    }
  }, [userId, storeId]);

  const loadOrCreateCart = async (uid: string, sid: string) => {
  ...
  return { cart, addItem, removeItem, updateItemQty, setCartStatus };
}
